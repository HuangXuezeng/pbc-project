...
export default {
  ...
  methods:{
    ...
    getPersonList(){
      let queryData= {}
      queryData.deptCode = localStorage.getItem('depCode2')
      getPerson(queryData).then(res=>{
        // console.log(res.data.a01s)
        let itemList = []
        if(res.data.a01s == ''){
    ...
    passInfo(item){
      // console.log(item);
      localStorage.setItem('depCode3',item.deptCode)
      this.$router.push({name:'achievementsOtherInNextInfo'})
    },
    //点击人
    passIn(item){
      // console.log(item);
