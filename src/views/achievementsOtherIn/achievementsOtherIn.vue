...
export default {
  ...
  methods:{
    ...
    getPersonList(){
      let queryData= {}
      queryData.deptCode = localStorage.getItem('depCode1')
      // queryData.deptCode = this.$route.params.deprtCode
      getPerson(queryData).then(res=>{
        ...
        if(res.data.a01s == ''){
          ...
          for(let i in res.data.a01s){
            ...
            if(res.data.a01s[i].potoURL == undefined){
    ...
    passInfo(item){
      // console.log(item);
      localStorage.setItem('depCode2',item.deptCode)
      this.$router.push({name:'achievementsOtherInNext'})
    },
    //点击人信息
    passIn(item){
      // console.log(item);
